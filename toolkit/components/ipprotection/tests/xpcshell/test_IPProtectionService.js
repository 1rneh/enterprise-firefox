...
/**
 * Tests that the browser.ipProtection.bandwidth.enabled pref is set from the
 * entitlement's limitedBandwidth field.
 */
add_task(async function test_bandwidth_enabled_pref_tracks_entitlement() {
  ...
  IPPDummyAuthProvider.resetEntitlement();
  await IPPDummyAuthProvider.updateEntitlement();
  Assert.equal(
